            ...
            for sensor in OximeterOptionsParser.SENSOR_LIST:
                ...
                if (not isinstance(sensor_config, dict)):
                    ...
                    if (not isinstance(sensor_config[OximeterOptionsParser.GATT_CHARACTERISTIC_NAME_ENTRY], str)):
                        raise TypeError(f'"{OximeterOptionsParser.GATT_CHARACTERISTIC_NAME_ENTRY}" should be "float", got {type(sensor_config[OximeterOptionsParser.GATT_CHARACTERISTIC_NAME_ENTRY])}')
                    if (not isinstance(sensor_config[OximeterOptionsParser.GATT_CHARACTERISTIC_UUID_ENTRY], str)):
                        raise TypeError(f'"{OximeterOptionsParser.GATT_CHARACTERISTIC_UUID_ENTRY}" should be "float", got {type(sensor_config[OximeterOptionsParser.GATT_CHARACTERISTIC_UUID_ENTRY])}')
                
                    # don't blindly import other keys if configured
                    self.sensors[sensor] = {
                        entry: sensor_config[entry] 
                        for entry in [
                            OximeterOptionsParser.GATT_CHARACTERISTIC_PERIOD_ENTRY,
                            OximeterOptionsParser.GATT_CHARACTERISTIC_NAME_ENTRY,
                            OximeterOptionsParser.GATT_CHARACTERISTIC_UUID_ENTRY
                        ]
                    }
            ...
            for flag in OximeterOptionsParser.FLAG_LIST:
                ...
                if (not isinstance(sensor_config, dict)):
                    ...
                    if (not isinstance(flag_config[OximeterOptionsParser.GATT_CHARACTERISTIC_NAME_ENTRY], str)):
                        raise TypeError(f'"{OximeterOptionsParser.GATT_CHARACTERISTIC_NAME_ENTRY}" should be "float", got {type(flag_config[OximeterOptionsParser.GATT_CHARACTERISTIC_NAME_ENTRY])}')
                    if (not isinstance(flag_config[OximeterOptionsParser.GATT_CHARACTERISTIC_UUID_ENTRY], str)):
                        raise TypeError(f'"{OximeterOptionsParser.GATT_CHARACTERISTIC_UUID_ENTRY}" should be "float", got {type(flag_config[OximeterOptionsParser.GATT_CHARACTERISTIC_UUID_ENTRY])}')
